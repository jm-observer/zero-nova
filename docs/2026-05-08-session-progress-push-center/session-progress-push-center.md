...
| Plan | 描述 | 依赖 | 顺序 | 状态 |
|------|------|------|------|------|
| Plan 1 | 审计并补齐 Session 进度恢复所需的持久化快照边界 | 无 | 1 | 待开始 |
| Plan 2 | 实现后端 Push Center，集中管理连接、订阅关系与事件广播 | Plan 1 | 2 | 待开始 |
| Plan 3 | 前端刷新恢复：重连后主动拉取当前 Session 进度并恢复页面状态 | Plan 1, Plan 2 | 3 | 待开始 |
...
- 待定项：前端恢复范围是否只覆盖当前 Session，还是覆盖所有正在运行的 Session
