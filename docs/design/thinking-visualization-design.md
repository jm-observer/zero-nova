...
4. **Phase 4 - 多模型扩展 (归一化层)**
   - 详细设计见：[multi-provider-reasoning-normalization.md](file:///d:/git/zero-nova/docs/design/multi-provider-reasoning-normalization.md)
   - 实现 OpenAI `reasoning_content` 适配（o1/o3-mini）
   - 实现 DeepSeek-R1 `reasoning_content` 适配
   - [可选] 兼容 Groq 等其他 Provider 的 reasoning 别名
