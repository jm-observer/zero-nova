...
use crate::gateway::protocol::{
    ...
    ProgressEvent, Session as SessionProtocol,
};
...
mod tests {
    ...
    #[test]
    fn app_error_maps_to_legacy_gateway_error_code() {
        let message = app_event_to_gateway(AppEvent::Error("boom".to_string()), "req-1", "session-1");

        match message.envelope {
            MessageEnvelope::Error(payload) => {
                assert_eq!(payload.message, "boom");
                assert_eq!(payload.code.as_deref(), Some("AGENT_RUNTIME_ERROR"));
            }
            _ => panic!("expected MessageEnvelope::Error"),
        }
    }
}
